define(function(require, exports, module) {
    ...
    var TreeTemplate = Backbone.Model.extend({
        ...
        initialize: function() {
            _(this).bindAll('generate');
        },

        generate: function(chart, container) {

            // Brian's code, reset
            if(this.viewPaper) {
                console.log('ok');
                this.viewPaper.remove();
                this.viewPaper = null;
            }

            if(innerContainer) {
                innerContainer.parentNode.removeChild('div');
            }
            ...
            this.viewPaper = Raphael(innerContainer,windowWidth,windowHeight);
            $(container).css(
            ...
            ******************************************************/
            ...
            Tree.title = this.viewPaper.text(0,0).attr({'text': title,'text-anchor': 'start', 'font-family': "Helvetica Neue",'font-weight': 100, 'fill': qsFontFill, 'font-size': 80});
            ...
            layer.width = ( ( layer.length - 1 ) * spacingX );
                ...
                _.each(layer, function (node) {
                    ...
                    node.textBox = this.viewPaper.text(0,0).attr({'text-anchor': 'start', 'font-family': "Helvetica Neue",'font-weight': 100, 'fill': 'black', 'font-size': 18});
                    ...
                    node.designBox = this.viewPaper.rect(0,0,node.textBox.getBBox().width + nodeBuffer*2,node.textBox.getBBox().height + nodeBuffer*2);
            ...
            windowWidth += 100;
            this.viewPaper.setSize(windowWidth,currentY);
            ...
            var setNodePositions = function (node) {
                ...
                if(testing) {
                this.viewPaper.rect(node.zoneXpos + 2,Tree.layers[node.layer].startY,node.zone - 4,1);
                this.viewPaper.rect(node.zoneXpos+node.zone / 2,Tree.layers[node.layer].startY,1,4);
                this.viewPaper.rect(node.textBox.getBBox().x,node.textBox.getBBox().y,5,5);
                this.viewPaper.rect(node.designBox.attr('x'),node.designBox.attr('y'),5,5);
                }
                _.each(node.adjacent, function (adj) {
            ...
            var routePaths = function (node) {
                _.each(node.adjacent, function (adj) {
                    ...
                    var newPath = this.viewPaper.path(pathString).attr({'stroke': '#999'}).toBack();
                    newPath.child = adj;
